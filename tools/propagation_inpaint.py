    ...
    while masked_frame_num > 0:
        ...
        if iter_num == 0:
            ...
            if args.FIX_MASK:
                ...
                label = cv2.imread(
                    os.path.join(mask_root, '%05d.png' % (flow_start_no)), cv2.IMREAD_UNCHANGED)
            print(flow_start_no)        
        ...
        for th in range(1, frames_num-1):
            ...
            if iter_num == 0:
                if not args.FIX_MASK:
                    label = cv2.imread(
                        os.path.join(mask_root, '%05d.png' % (th)), cv2.IMREAD_UNCHANGED)
                else:
                    label = cv2.imread(
                        os.path.join(mask_root), cv2.IMREAD_UNCHANGED)
                label = cv2.resize(label, (image.shape[1], image.shape[0]), interpolation=cv2.INTER_NEAREST)
            ...
            temp1 = flo.get_warp_label(flow1, flow2,
                                        ...
                                        th=th_warp)
            #print(temp1)
            temp3 = flo.get_warp_label(flow3, flow4,
                                        temp1,
                                        th=th_warp)
            ...
            temp6[0][...,0] = temp2
            #print(temp5.shape)
            temp4 = flo.get_warp_label(flow3, flow4,
                                        temp6[0],
                                        th=th_warp,
                                        value=-1)[..., 0]
        ...
        for th in range(frames_num - 2, 0, -1):
            ...
            temp6[0][...,1] = temp2
            #print(temp5.shape)
            temp7 = flo.get_warp_label(flow3, flow4,
                                        temp6[0],
                                        th=th_warp,
                                        value=-1)[..., 1]
